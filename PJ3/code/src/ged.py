...
#新增部分：参数topk_paramter，默认为1与原本的Noah一样返回GED，设置为整数值表示返回topk相似的GED
def graph_edit_distance(u, v, lower_bound, beam_size, start_node = None, topk_paramter = 1):
    # Partial edit path
    open_set = []
    cost_open_set = []
    partial_cost_set = []
    ...
    while cost_open_set:
        ...
        if not unprocessed_u and not unprocessed_v:
            if topk_paramter == 1:
                ...
                k_count += 1
                top_k_result.append((min_path, cost, cost_list, call_count, time_count, path_idx_list))
                if k_count == topk_paramter:
                    return top_k_result
...
def main():
    ...
    #     min_path, cost, cost_list, call_count, time_count, path_idx_list = graph_edit_distance(g1, g2, lower_bound, beam_size, start)
    #     if cost < cost_min:
    #         start_min = [start]
    #         cost_min = cost
    #     elif cost == cost_min:
    #         start_min.append(start)
    # print (start_min, cost_min)

    # Compute GED
    """
    Optimizing A* with predicted start nodes.
    You can manually close by commenting the code followed.
    """
    with open('start_node_model.pkl', 'rb') as f:
        clf = pickle.load(f)
    pre_model = torch.load("pretrained_model.pkl")
    data = transfer_to_torch(list(g1.nodes()),list(g2.nodes()),g1,g2)
    embedding1, embedding2 = pre_model.forward(data)
    embedding = torch.cat((torch.t(embedding1),torch.t(embedding2)),dim=1).view(1,-1).detach().cpu().numpy().tolist()
    start_node = clf.predict(embedding)
