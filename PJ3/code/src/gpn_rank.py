...
from utils_rank import process_pair, process_pair_test, process_file, process_file_without_path, calculate_loss, get_ged, calculate_ranking_correlation, calculate_prec_at_k, loadGXL, unprocessed_cost #, gen_output
...
class GPNTrainer(object):
    ...
    def load_ged(self):
        ...
        """
        self.ged_dir = glob.glob(self.args.training_graphs + "*train_ged.txt")
        graph1 = []
        graph2 = []
        ged = []
    ...
    def load_ged_test(self):
        ...
        """
        self.ged_dir = glob.glob(self.args.testing_graphs + "*train_test_ged.txt") #learning ability: self.args.testing_graphs 
        graph1 = []
        graph2 = []
        ged = []
        fin = open(self.ged_dir[0])
    ...
    def create_data_set(self):
        ...
        if self.args.global_labels == 'Syn.pkl':
            ...
            for i in range(len(self.training_graphs)):
                if training_source[0] != training_target[i]:
                    self.training_graphs_set.append([training_target[i],training_source[0]])
    ...
    def transfer_to_torch_test(self, g1, g2, ged, graph_pair):
        ...
        real_ged = ged.loc[ged['graph2']==os.path.basename(graph_pair[0])]
        real_ged = real_ged[real_ged['graph1']==os.path.basename(graph_pair[1])]
        if real_ged.empty:
            return None
        else:
            real_ged = real_ged.iloc[0]
            normalized_ged = int(real_ged['ged']) 
            target = torch.from_numpy(np.array(normalized_ged)).float()
            # target = torch.from_numpy(np.exp(-normalized_ged).reshape(1,1)).view(-1).float()
            new_data["target"] =  target.cuda()
            return new_data
    ...
    def cal_rank_loss(self, batch):
        ...
        self.optimizer.zero_grad()
        losses = 0
        score_predict = self.model(batch[0])
        score_real=[]
        score_real.append(batch[0]["target"])
        for i in range(len(batch)-1):
            data = batch[i+1]
            score_real.append(data["target"])
            prediction = self.model(data)
            score_predict = torch.cat((score_predict,prediction),0)
        
        score_real = torch.tensor(score_real).cuda()
        score_diff = torch.sigmoid(score_predict - score_predict.t())
        tij = (1.0 + torch.sign(score_real - score_real.t())) / 2.0
        losses = tij * torch.log(score_diff) + (1-tij)*torch.log(1-score_diff)
        diag = torch.diag(losses)
        a_diag = torch.diag_embed(diag)
        losses = losses - a_diag
        losses = -losses.sum()
        losses.backward(retain_graph = True)
        self.optimizer.step()
        loss = losses.item()
    ...
    def preprocess_data_file(self):
        ...
        self.load_ged()
        #print(self.training_graphs_set)
    ...
    def create_batches(self):
        ...
        training_source = self.training_data_set.copy()
        random.shuffle(self.training_data_set)
        batches = [training_source[graph:graph+self.args.batch_size] for graph in range(0, len(self.training_data_set), self.args.batch_size)]
        return batches
    ...
    def fit(self):
        ...
        """
        print("\nModel training.\n")
        if os.path.exists('/home/zhangji/Noah-GED/results/model.pkl'):
            self.model = torch.load('/home/zhangji/Noah-GED/results/model.pkl')
        ...
        for epoch in epochs:
            ...
            for index, batch in tqdm(enumerate(batches), total=len(batches), desc = "Batches"):
                loss_score = self.cal_rank_loss(batch)
                main_index = main_index + len(batch)
                self.loss_sum = self.loss_sum + loss_score
                loss = self.loss_sum/main_index
                epochs.set_description("Epoch (Loss=%g)" % round(loss,5))
        torch.save(self.model,'/home/zhangji/Noah-GED/results/model.pkl')
        print("\nModel saved.\n")
    ...
    def score(self):
        ...
        if self.args.combinatorial == True:
           ...
           # Evaluation for learning based methods.
            ...
            fea = 0
            count = 0
            #print(len(self.training_graphs)*len(self.testing_graphs))
            target_batch = []
            prediction_batch = []
            target_b = np.empty(len(self.testing_graphs))
            prediction_b = np.empty(len(self.testing_graphs))
            for i in range(len(self.testing_graphs)):
                graph_pair = self.testing_graphs[i], self.training_graphs[0]
                g1,g2 = process_pair_test(graph_pair, self.super_label)
                if (g1) == (None):
                    print('none g1')
                    continue
                ...
                if data == None:
                    ...
                    continue
                target = data["target"]
                prediction = self.model(data).detach() * self.higher_bound(g1, g2)
                target_b[i] = target
                prediction_b[i] = prediction.detach()
                target_batch.append((i,target))
                prediction_batch.append((i,prediction))
            rho_list = []
            rho_list.append(calculate_ranking_correlation(spearmanr, prediction_b, target_b))
            self.rho = np.mean(rho_list)
            print("rho:",self.rho)
            target_batch.sort(key = lambda x:x[1],reverse = True)
            per = int(len(target_batch)/10)
            print("per",per)
            target_batch = target_batch[:50]
            prediction_batch.sort(key = lambda x:x[1],reverse = True)
            prediction_batch = prediction_batch[:50]
            #print(target_batch)
            #print(prediction_batch)
            target_100=set([target_batch[i][0] for i in range(len(target_batch))])
            prediction_100=set([prediction_batch[i][0] for i in range(len(prediction_batch))])
            acc=0
            
            for item in prediction_100:
                if item in target_100:
                    acc+=1
            print("top-50 rank accuracy:",round(acc/50,5))
