...
from .match import get_subgraph_macth,graph2matchfile
from .match import enumerate_subgraph,generate_candidates,filter_candidate
...
class gSpan(object):
    ...
    def __init__(self,
        ...
        #根据输入的文件路径生成对应的data_graph,必须为supgraph match的格式
        self.data_graph_path = self._database_file_name + '.DPiso'
    ...
    def _read_graphs(self):
        ...
        #将待挖掘的图转化为DPiso的可以识别的格式
        graph2matchfile(self.graphs[0],self.data_graph_path)
        return self
    ...
    def run(self):
        ...
        for gid, g in self.graphs.items():
            for vid, v in g.vertices.items():
                ...
                for e in edges:
                    ...
                    #记录从边到图的映射 vevlb -> PDF(gid,e,None)
        ...
        for vevlb, projected in root.items():
            ...
            #更新DFSCode，然后进行子图挖掘
            self.pattern_grow(DFSedge(0, 1, vevlb))
            self._subgraph_mining(projected)
            self._DFScode.pop()

    def _get_support(self, projected):

        #使用set将gid元素去重，使得每个图只计数一次
    ...
    def get_support_by_enumerate(self):
        G = self.graphs[0]
        Q = self._DFScode.to_graph(gid=VACANT_GRAPH_ID,
                                   is_undirected=self._is_undirected)
        Candidate = generate_candidates(G,Q)
        Candidate = filter_candidate(G,Q,Candidate,self._min_support)
        self._support = enumerate_subgraph(G,Q,Candidate)

    #调用DPiso作为子程序计算支撑度
    def get_suupport_by_DPsio(self):
        #将DFS code转化为graph
        
        g = self._DFScode.to_graph(gid=VACANT_GRAPH_ID,
                                   is_undirected=self._is_undirected)
        
        #需要将其输入到一个文件中后，调用match函数读取该文件返回support的结果
        query_graph_path = 'temp'
        graph2matchfile(g,query_graph_path)
        self._support = get_subgraph_macth(self.data_graph_path, query_graph_path,binary_path='gspan\DPiso.exe')

    def pattern_grow(self,edge:DFSedge):
        self._DFScode.append(edge)
        #TODO 添加动态图更新的部分，记录之前的Candidate进行更新操作
    ...
    def _subgraph_mining(self, projected):
        ...
        #self.get_suupport_by_DPsio()
        self.get_support_by_enumerate()
